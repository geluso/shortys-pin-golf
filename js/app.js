...
function initScoreForm({ form, tbody, messageEl, onSaved }) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      const entry = await api(appPath('/api/entries/index.php'), {
        method: 'POST',
...
function initEditForm({ form, tbody, messageEl, entry }) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      await api(appPath(`/api/entries/index.php/${encodeURIComponent(entry.id)}`), {
        method: 'POST',
...
async function loadLeaderboard(listEl, options = {}) {
  ...
  const entries = await api(appPath('/api/entries/index.php'));
  ...
  if (shown.length === 0) {
    ...
    shown.forEach((entry) => {
      ...
      link.href = appPath(`/entry/index.php?id=${encodeURIComponent(entry.id)}`);
...
async function loadEntry(id) {
  ...
  return api(appPath(`/api/entries/index.php/${encodeURIComponent(id)}`));
}
